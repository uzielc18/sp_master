...
public function guias_mes($Y='',$m='')
	{
		...
		$mes_activo=$this->mes_activo=date('m');
		if($m!='')$mes_activo=$this->mes_activo=$m;
		$this->titulo='Guias del mes '.date("F");
		$this->url='crearsalidas';
		$documentos= new Tesdocumentos();
		$id=15;
		$doc= $documentos->find_first((int)$id);
...
public function factura_adelanto($id=0)
	{
		...
		if (Input::hasPost('salida')) 
		{
            ...
            if ($salidas->save())
			{
				...
				{//Flash::warning('No se Pudieron Guardar los Datos...('.$factura_a->montototal.')!!!');
				}
...
{	
	...
	*/
	$salidas= new Tessalidas();
	$this->salidas= $salidas->find('conditions: (serie="F001" OR serie="001" OR serie = "F002") AND estadosalida!="TERMINADO" AND npedido like "SV%" AND estado=1 AND tesdocumentos_id='.Session::get('DOC_ID').' AND aclempresas_id='.Session::get("EMPRESAS_ID"),'order: fecha_at DESC');
	Session::delete("SALIDA_ID");

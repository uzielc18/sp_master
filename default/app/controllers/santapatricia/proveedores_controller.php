...
class ProveedoresController extends AdminController {
	...
	public function crear($campo,$direccion,$page=1)
	{
	try {
		$this->campo=$campo;
		$this->direccion=$direccion;
		...
		$this->empresas=$emp->find_first('columns: nombre','conditions: id='.Session::get('EMPRESAS_ID'));
             ...
             if (Input::hasPost('tesdatos')) {
                ...
                if ($dat->save()) {
                    ...
                    Flash::valid('El Proveedor Ha Sido Agregado Exitosamente...!!!');
                    Aclauditorias::add("Agregó al Proveedor {$dat->nombre} al sistema", 'tesdatos');
	...
	public function direccionfiscal($id=0,$dato_id=0)
	{
		...
		$direcciones= new Tesdatosdirecciones();
		$this->direcciones=$direcciones->find('conditions: tesdatos_id='.$dato_id);
	...
	public function editar($campo,$direccion,$id,$page=1) {
        try {
			$this->campo=$campo;
			$this->direccion=$direccion;
			$this->page=$page;
			...
			*/
			...
			$this->titulo='Modificar al Proveedor ('.$this->tesdatos->razonsocial.')';
            if (Input::hasPost('tesdatos')) {
					$dat->testipodatos_id=$this->proveedor;
					$dat->userid=Auth::get('id');
					$dat->aclempresas_id=Session::get('EMPRESAS_ID');
				...
				if ($dat->update(Input::post('tesdatos'))) {
                    ...
                    Flash::valid('El Proveedor Ha Sido Actualizado Exitosamente...!!!');
	...
	public function activar($campo,$direccion,$id,$page=1) {
        try {
            ...
            if (!$dat->find_first($id)){ //si no existe el usuario
                ...
                Flash::valid("El Proveedor<b>{$dat->razonsocial}</b> Esta ahora <b>Activo</b>...!!!");
                Aclauditorias::add("Colocó al Proveedor {$dat->razonsocial} como activo", 'testproductos');
            } else {
                Flash::warning('No se pudo Activar el Proveedor!!!');
            }
        } catch (KumbiaException $e) {
            View::excepcion($e);
        }
     ...
     public function desactivar($campo,$direccion,$id,$page=1) {
        try {
            ...
            if (!$dat->find_first($id)){ //si no existe el usuario
                ...
                Flash::valid("El Proveedor <b>{$dat->razonsocial}</b> Esta ahora <b>Inactivo</b>...!!!");
                Aclauditorias::add("Colocó el Proveedor {$dat->razonsocial} como inactivo", 'testproductos');
            } else {
                Flash::warning('No se Pudo Desactivar el Proveedor...!!!');
            }
        } catch (KumbiaException $e) {
            View::excepcion($e);
        }
	...
	public function borrar($campo,$direccion,$id=0,$page=0)
	{
        try {
            ...
            if (!$dat->find_first($id)) { //si no existe
                ...
                Flash::valid("EL Proveedor <b>{$dat->razonsocial}</b> fué Eliminado...!!!");
                Aclauditorias::add("EL Proveedor {$dat->razonsocial} del sistema", 'tesdatos');
            } else {
                Flash::warning("No se Pudo Eliminar EL Proveedor <b>{$dat->razonsocial}</b>...!!!");
            }
        } catch (KumbiaException $e) {
            View::excepcion($e);
        }
	...
	public function resultados() 
	{
		$q=$_GET['q'];
		$obj = new Tesdatos();
		$results = $obj->find('columns:id,codigo,razonsocial,ruc,departamento,provincia,distrito,pais dni','conditions: testipodatos_id='.$this->proveedor.' and CONCAT_WS(" ",codigo,razonsocial,ruc,departamento,provincia,distrito,pais) like "%'.$q.'%" AND aclempresas_id='.Session::get('EMPRESAS_ID'));
	...
	public function ver($campo,$direccion,$id=0,$page=1)
	{
        try {
			...
			$this->empresas=$emp->find_first('columns: nombre','conditions: id='.$this->empresa);
            $this->tesdatos = $dat->find_first($id);
			$this->tescontactos=FALSE;
			if($contactos->exists('estado=1 and activo=1 and tesdatos_id='.$id)){
			$this->tescontactos=$contactos->find_first('conditions: estado=1 and activo=1 and tesdatos_id='.$id);
			}
			$this->titulo='Detalle al Proveedor ('.$this->tesdatos->razonsocial.')';
        } catch (KumbiaException $e) {
            View::excepcion($e);
        }
	}
	...
	public function proveedor() 
	{
		...
		$q=$_GET['q'];
		$obj = new Tesdatos();
		$results = $obj->find('columns:id,codigo,razonsocial,ruc,departamento,provincia,distrito,pais dni','conditions: estado=1 AND activo=1 AND testipodatos_id='.$this->proveedor.' and CONCAT_WS(" ",codigo,razonsocial,ruc,departamento,provincia,distrito,pais) like "%'.$q.'%" AND aclempresas_id='.Session::get('EMPRESAS_ID'));
		foreach ($results as $value)
		{
			$id=$value->id;
			$name=$value->codigo_ant.' - '.$value->razonsocial;
			$json = array();
			$json['id'] =$id;
			$json['name'] = $name;
			$this->data[] = $json;
	...
	{
		$datos= new Tesdatos();
		$dats=$datos->find('conditions: ISNULL(sufijo) AND testipodatos_id=1 AND estado=1 AND activo=1');
...
{
	$datos= new Tesdatos();
		$dats=$datos->find('conditions: testipodatos_id=1 and aclempresas_id=1');
		$n=0;
		foreach($dats as $dat)
		{
			$n++;
...
public function reporte_proveedores($campo='codigo',$direccion="ASC")
{
	$this->campo=$campo;
	$this->direccion=$direccion;
	$tesdatos= new Tesdatos();
	$orden=$campo.' '.$direccion;
	$this->datos=$tesdatos->find('conditions: aclempresas_id='.Session::get('EMPRESAS_ID').' AND testipodatos_id=1','order: '.$orden);
}
public function reporte_proveedores_imprimir($campo='codigo',$direccion="ASC")
{
	$this->campo=$campo;
	$this->direccion=$direccion;
	$tesdatos= new Tesdatos();
...
public function modificar_direcciones($valid=0)
{
	if($valid==43408841){
	$datos=new Tesdatos();
	$dts=$datos->find_all_by_sql('SELECT distrito_ant,distrito,departamento,provincia,direccion,id FROM `tesdatos` WHERE aclempresas_id='.Session::get('EMPRESAS_ID').' AND testipodatos_id=1');
	foreach($dts as $dt):
	$direcciones= new Tesdatosdirecciones();
...
public function listar_direcciones()
{
	...
	$direcciones= new Tesdatosdirecciones();
	$this->direcciones=$direcciones->getDirecciones(1);
}
